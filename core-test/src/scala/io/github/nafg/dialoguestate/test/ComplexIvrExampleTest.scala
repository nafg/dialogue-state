...
object ComplexIvrExampleTest extends ZIOSpecDefault {
  ...
  private class ComplexIvrExampleService(val authenticatedUserRef: Ref[Option[User]]) {
    ...
    object mainMenu                    extends CallTree.Gather(numDigits = Some(1), timeout = 10) {
      override def message: CallTree.NoContinuation =
        CallTree.Say(
      ...
      override def handle: String => CallTree.Callback = {
        case "1" => authenticatedUserRef.get.someOrFailException.asRightError.map(BalanceMenu(_))
    ...
    object recentTransactionsMenu extends CallTree.Gather(timeout = 10) {
      override def message: CallTree.NoContinuation =
        CallTree.Say(
    ...
    object fundsTransferMenu extends CallTree.Gather(timeout = 10) {
      override def message: CallTree.NoContinuation =
        CallTree.Say(
    ...
    object customerServiceMenu extends CallTree.Gather(timeout = 10) {
      override def message: CallTree.NoContinuation =
        CallTree.Say(
  ...
  private object ComplexIvrExampleService                                             {
    val layer = ZLayer {
      for {
        ref <- Ref.make(Option.empty[User])
      } yield new ComplexIvrExampleService(ref)
    }
  }

  override def spec: Spec[TestEnvironment, Any] =
    suite("Complex IVR Test")(
      test("can authenticate and check balance") {
        for {
          service <- ZIO.service[ComplexIvrExampleService]
          tester  <- CallTreeTester(service.welcomeMenu)
          ...
          _       <- tester.expect("Authentication successful", "Main Menu")
      ...
      test("can handle locked account") {
        for {
          ...
          _       <- tester.expect("We're sorry, but your account has been locked", "Thank you for calling SecureBank")
        } yield assertCompletes
      },
      test("can transfer funds to another account") {
        for {
          ...
          _       <- tester.expect("To transfer funds to another customer", "Please enter the 8-digit account number")
          _       <- tester.sendDigits("87654321")
          _       <- tester.expect("Please enter the amount to transfer")
          _       <- tester.sendDigits("100")
          _       <- tester.expect("You are about to transfer $100.00", "Press 1 to confirm")
          _       <- tester.sendDigits("1")
      ...
      test("can leave a message for customer service") {
        for {
          ...
          _       <- tester.expect("Thank you for your message", "A customer service representative will contact you")
          _       <- tester.sendDigits("0")
          _       <- tester.expect("Thank you for using SecureBank telephone banking")
        } yield assertCompletes
      },
      test("can view recent transactions") {
        for {
          ...
          _       <- tester.expect("Authentication successful", "Main Menu")
          _       <- tester.sendDigits("2")
          _       <- tester.expect("Your recent transactions", "Grocery Store", "Gas Station", "Online Retailer")
          _       <- tester.sendDigits("1")
          _       <- tester.expect("Main Menu")
        } yield assertCompletes
      }
    ).provide(ComplexIvrExampleService.layer)
}
